...
Keep the original ISMN, AmeriFlux, and local files in a local source archive, along with source names, versions, citations, and any use restrictions. The ISMN importer reads the supplied header+values export, pairs temperature and moisture streams by exact depth and instrument identity where possible, and writes hourly UTC CSVs. It preserves original flags in compressed sidecars without making a new QA/QC decision. The independent AmeriFlux importer reads BASE-BADM site archives, BIF coordinates and UTC offsets, and the separately published Measurement Height table. It keeps source-specific IDs and pairs temperature/moisture only at a uniquely matching documented soil depth. The importer for locally collected data remains future work.
...
Source-specific private site/sensor tables are stored outside Git on the private data volume, while `metadata/sensors.csv` remains the public schema template. Site tables carry coordinates; sensor tables repeat coordinates and available depths with stable site IDs. Pairing reports preserve original file/column provenance and unresolved cases. Soil-moisture-only streams are indexed but not turned into temperature-based classifier inputs. The full ISMN export has been harmonized on the private Expansion volume. AmeriFlux site coordinates can have multiple BIF entries; such sites are flagged for review before land-cover screening.
...
Unavailable measurements are `NaN`. If an hour is absent from the source, include an all-`NaN` hourly row and retain the original source record separately. ISMN timestamps are already UTC hourly. AmeriFlux BASE timestamps are local standard time; its importer uses the BIF fixed UTC offset and valid-minute-weighted hourly means. The validator checks UTC timestamps and consecutive hourly rows; it does not yet perform new anomaly QA/QC.

The single `raw_value` column covers one native channel per sensor, with its name and unit in metadata. The inspected ISMN and AmeriFlux exports contain calibrated soil moisture and temperature but no raw frequency/count signal, so `raw_value` is `NaN`. If another source supplies multiple useful raw channels, we should extend the schema explicitly, for example with a separate long-format raw-observations table.

## 3. Validate and enrich

Before modeling, verify unique sensor IDs, coordinates, units, depth, hourly continuity, duplicate timestamps, and missingness. Keep source quality flags and coverage information for later filtering. The ISMN importer preserves flagged finite values; the AmeriFlux importer preserves finite BASE values except the documented `-9999` missing marker. Neither adds new anomaly QA/QC. Add optional modeled soil variables to metadata with provenance. Generate a per-sensor coverage report so we know whether temperature, moisture, or raw measurements are sufficient for classification. AmeriFlux variables may include site-level aggregates alongside individual probes; they must not all count as independent sensors in grid summaries.
...
| ISMN harmonization | Full northern export harmonized on private Expansion volume | Review ambiguous depth pairing and later QA/QC |
